...
public class NewCalculator extends JFrame implements ActionListener{
    ...
    private void dealWithFunction(String label) {
        ...
        try {
            ...
            if(label.equals("ln") && temResult != 0) {
                log_e_factory f=new log_e_factory();
                temResult = f.calutation().operation(temResult);
            }
            else if(label.equals("√x")){
                sqrt_factory f=new sqrt_factory();
                temResult = f.calutation().operation(temResult);
            }
            else if(label.equals("1/x") && temResult != 0) {
                reciprocal_factory f=new reciprocal_factory();
                temResult = f.calutation().operation(temResult);
            }
            else if(label.equals("sin")) {
                sine_factory f= new sine_factory();
                temResult = f.calutation().operation(temResult);
            }
            else if(label.equals("cos")) {
                cosine_factory f = new cosine_factory();
                temResult = f.calutation().operation(temResult);
            }
            else if(label.equals("tan")) {
                tangent_factory f = new tangent_factory();
                temResult = f.calutation().operation(temResult);
            }
